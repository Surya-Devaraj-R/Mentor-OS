...
public static class InterviewPrepEndpoints
{
    public static void MapInterviewPrepEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/questions", async (QuestionType? type, AppDbContext db, ProgressService progress) =>
        {
            var query = db.InterviewQuestions.AsQueryable();
            ...
            return Results.Ok(questions.Select(q => new InterviewQuestionDto(
                q.Id, q.QuestionType, q.Title, q.PromptText, q.SuggestedApproach, q.SampleAnswer,
                q.SortOrder, completedIds.Contains(q.Id))));
        });

        group.MapGet("/questions/{id:int}", async (int id, AppDbContext db, ProgressService progress) =>
        {
            var question = await db.InterviewQuestions.FindAsync(id);
        ...
        group.MapGet("/coding-challenges", async (AppDbContext db) =>
        {
            ...
            var dtos = exercises.Select(e =>
            {
                ...
                return new ExerciseSummaryDto(
                    ...
                    e.IsInterviewChallenge, e.Language, latest?.SelfAssessment);
            });
